...
use integrity_lineage_models::models;
...
/// * `signer` - Signer implementation used to produce the signature
///
/// # Returns
...
pub async fn sign_dsse(
    ...
    let signature = Signature { keyid, sig };
...
pub async fn sign_integrity_statement_dsse(
    ...
    let envelope = sign_dsse(payload, payload_type, signer).await?;
